...
const team = [
  { name: "Mariam Khan", title: "TRAINEE SOLICITOR", bio: "A dedicated and diligent member of our team, Mariam Khan embodies the values and ethos of Maurice Andrews Solicitors. She consistently demonstrates a keen understanding of criminal law, combined with a passion to serve and represent our clients with integrity. As a Trainee Solicitor, Mariam's fresh perspective and commitment to continuous learning make her a valuable asset to our team." },
  { name: "Mazar Iqbal", photo: "/assets/other-01.mazar.jpg", bio: "Joining Maurice Andrews Solicitors in 2018, Mr. Iqbal has established himself as an integral member of our legal team." },
  { name: "Tabrace Hussain", photo: "/assets/P1345612-1.1d45a86b.jpg", bio: "Tabrace Hussain is an integral part of the Maurice Andrews Solicitors team, consistently delivering high-quality legal support." },
];
    ...
    <>
      ...
      <main>
        ...
        <section
          ...
          style={{ backgroundImage: "url('/assets/hero-two-men.jpg')", minHeight: '420px' }}
        >
        ...
        {/* TEAM */}
        <section className="bg-white px-4 py-14"><div className="mx-auto max-w-[1240px]"><h2 className="mb-2 text-center text-2xl font-bold text-primary lg:text-3xl">Our team</h2><p className="mb-8 text-center font-semibold"><Link href="/ourTeam" className="inline-flex items-center gap-1 text-primary hover:underline">See all <ArrowRight size={16} /></Link></p><div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">{team.map(m => <Link key={m.name} href={"/ourTeam?member=" + encodeURIComponent(m.name)} className="group text-center"><div className="relative mx-auto mb-4 overflow-hidden bg-[#E8F1FE]">{m.photo ? <Image src={m.photo} alt={m.name} width={280} height={320} className="w-full object-cover" /> : <div className="flex aspect-[4/5] w-full items-center justify-center bg-[#E8F1FE]"><svg width="100" height="130" viewBox="0 0 120 150" fill="none" className="text-[#1C2946]/30"><circle cx="60" cy="40" r="30" fill="currentColor"/><path d="M10 150c0-40 22.4-60 50-60s50 20 50 60" fill="currentColor"/></svg></div>}<div className="absolute inset-0 bg-[#1C2946]/0 transition-colors duration-300 group-hover:bg-[#1C2946]/40" /></div><h3 className="mb-1 text-lg font-bold text-primary">{m.name}</h3>{m.title && <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-[#8B9DC3]">{m.title}</p>}<p className="text-[13px] leading-relaxed text-grey">{m.bio}</p></Link>)}</div></div></section>
